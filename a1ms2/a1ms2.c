...
int main(void)
{
    ...
    if (type == 'A')
    {
        account.Acc_type = "AGENT";
    }
    else if (type == 'C')
    {
        account.Acc_type = "CUSTOMER";
    }
    printf("\n");
    // ---------------------------------------------------------
    ...
    getCString(person.Full_name, 0, 30);
    //Birt Year
    int getYear;
    int min_Y, max_Y;
    getYear = currentYear();
    ...
    person.YOB = getIntFromRange(min_Y, max_Y);
    //Household Income
    printf("Enter the household Income: $");
    person.household_income = getPositiveDouble();
    //Country
    printf("Enter the country (30 chars max.): ");
    getCString(person.country, 0, 30);
    printf("\n");
    // ---------------------------------------------------------
...
void displayAccount(const struct Account account, const struct UserLogin login, const struct Person person)
{
    ...
    printf(" %-9s", account.Acc_type);
    printf(" %-15s", person.Full_name);
    printf(" %5d", person.YOB);
    printf(" %11.2lf", person.household_income);
    printf(" %-10s", person.country);
    printf(" %-10s", login.Login_Name);
    printf(" %8s\n", login.password);
}
// Confirm MS1 stuff still works and grades can be applied to MS1
